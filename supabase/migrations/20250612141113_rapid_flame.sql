/*
  # Setup user registration trigger and demo data

  1. User Registration
    - Create function to handle new user registration
    - Set up trigger for automatic profile creation
    - Special handling for test user with complete setup

  2. Demo Data
    - Create sample posts that don't require user relationships
...
-- Create demo auth users first, then their profiles
-- This respects the foreign key constraint by creating auth.users entries first
DO $$
DECLARE
...
BEGIN
  -- First, create auth.users entries for demo users
  -- Note: In a real scenario, these would be created through Supabase Auth
  -- For demo purposes, we'll insert directly (this is not recommended in production)
  INSERT INTO auth.users (
    ...
    (demo_user_1, 'sarah.wilson@demo.com', crypt('demo123', gen_salt('bf')), now(), now(), now(), '{"provider": "email", "providers": ["email"]}', '{}', false, 'authenticated'),
    (demo_user_2, 'emily.chen@demo.com', crypt('demo123', gen_salt('bf')), now(), now(), now(), '{"provider": "email", "providers": ["email"]}', '{}', false, 'authenticated'),
    (demo_user_3, 'jessica.martinez@demo.com', crypt('demo123', gen_salt('bf')), now(), now(), now(), '{"provider": "email", "providers": ["email"]}', '{}', false, 'authenticated'),
    (demo_user_4, 'olivia.taylor@demo.com', crypt('demo123', gen_salt('bf')), now(), now(), now(), '{"provider": "email", "providers": ["email"]}', '{}', false, 'authenticated'),
    (demo_user_5, 'mike.davis@demo.com', crypt('demo123', gen_salt('bf')), now(), now(), now(), '{"provider": "email", "providers": ["email"]}', '{}', false, 'authenticated'),
    (demo_user_6, 'david.brown@demo.com', crypt('demo123', gen_salt('bf')), now(), now(), now(), '{"provider": "email", "providers": ["email"]}', '{}', false, 'authenticated'),
    (demo_user_7, 'chris.anderson@demo.com', crypt('demo123', gen_salt('bf')), now(), now(), now(), '{"provider": "email", "providers": ["email"]}', '{}', false, 'authenticated'),
    (demo_user_8, 'james.wilson@demo.com', crypt('demo123', gen_salt('bf')), now(), now(), now(), '{"provider": "email", "providers": ["email"]}', '{}', false, 'authenticated'),
    (demo_user_9, 'ryan.garcia@demo.com', crypt('demo123', gen_salt('bf')), now(), now(), now(), '{"provider": "email", "providers": ["email"]}', '{}', false, 'authenticated'),
    (demo_user_10, 'maya.patel@demo.com', crypt('demo123', gen_salt('bf')), now(), now(), now(), '{"provider": "email", "providers": ["email"]}', '{}', false, 'authenticated')
  ON CONFLICT (id) DO NOTHING;

  -- Now insert the profiles (these will satisfy the foreign key constraint)
