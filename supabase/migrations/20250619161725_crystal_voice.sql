/*
  ...
  1. Storage Setup
    ...
    - Create 'banner' bucket for banner/cover photos
  ...
  2. Security
    ...
    - Allow authenticated users to read bucket information
*/
...
-- Policy: Allow authenticated users to read bucket information
...
-- Policy: Allow authenticated users to upload banner images
CREATE POLICY "Users can upload their own banner"
  ON storage.objects
  FOR INSERT
  TO authenticated
  WITH CHECK (bucket_id = 'banner' AND auth.uid()::text = (storage.foldername(name))[1]);

-- Policy: Allow authenticated users to update their own banner images
CREATE POLICY "Users can update their own banner"
  ON storage.objects
  FOR UPDATE
  TO authenticated
  USING (bucket_id = 'banner' AND auth.uid()::text = (storage.foldername(name))[1]);

-- Policy: Allow authenticated users to delete their own banner images
